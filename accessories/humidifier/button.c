...
void button_intr_callback(uint8_t gpio) {
    ...
    if (gpio_read(button->gpio_num) == button->pressed_value) {
        ...
        if ((now - button->last_press_time) * portTICK_PERIOD_MS > button->very_long_press_time) {
          button->callback(button->gpio_num, button_event_very_long_press);
          return;
        }
...
int button_create(const uint8_t gpio_num, bool pressed_value, uint16_t long_press_time, uint16_t very_long_press_time, button_callback_fn callback) {
    ...
    button->very_long_press_time = very_long_press_time;
