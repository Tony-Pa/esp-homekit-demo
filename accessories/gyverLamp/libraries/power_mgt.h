...
///
uint32_t calculate_unscaled_power_mW(ws2812_pixel_t pixels[], uint16_t numLeds ) //25354
{
  ...
  for (int i = numLeds - 1; i >= 0; i--) {
    red32   += pixels[i].red;
    green32 += pixels[i].green;
    blue32  += pixels[i].blue;
  }
...
uint8_t calculate_max_brightness_for_power_mW(ws2812_pixel_t pixels[], uint16_t numLeds, uint8_t target_brightness, uint32_t max_power_mW) {
  uint32_t total_mW = calculate_unscaled_power_mW( pixels, numLeds);
