...
LIB8STATIC_ALWAYS_INLINE uint8_t scale8( uint8_t i, uint8_t scale)
{
...
LIB8STATIC_ALWAYS_INLINE uint8_t scale8_video( uint8_t i, uint8_t scale)
{
...
LIB8STATIC_ALWAYS_INLINE void nscale8_LEAVING_R1_DIRTY( uint8_t *i, uint8_t scale)
{
...
/// then explicitly call cleanup_R1.
LIB8STATIC_ALWAYS_INLINE uint8_t scale8_video_LEAVING_R1_DIRTY( uint8_t i, uint8_t scale)
{
    uint8_t j = (((int)i * (int)scale) >> 8) + ((i&&scale)?1:0);
    // uint8_t nonzeroscale = (scale != 0) ? 1 : 0;
    // uint8_t j = (i == 0) ? 0 : (((int)i * (int)(scale) ) >> 8) + nonzeroscale;
...
/// If you are doing several 'scale8_video's in a row, use this, and
/// then explicitly call cleanup_R1.
LIB8STATIC_ALWAYS_INLINE void nscale8_video_LEAVING_R1_DIRTY( uint8_t *i, uint8_t scale)
{
...
LIB8STATIC void nscale8x2( uint8_t *i, uint8_t *j, uint8_t scale)
{
    uint16_t scale_fixed = scale + 1;
...
LIB8STATIC void nscale8x2_video( uint8_t *i, uint8_t *j, uint8_t scale)
{
    uint8_t nonzeroscale = (scale != 0) ? 1 : 0;
...
LIB8STATIC_ALWAYS_INLINE uint16_t scale16by8( uint16_t i, uint8_t scale )
{
...
LIB8STATIC uint16_t scale16( uint16_t i, uint16_t scale )
{
